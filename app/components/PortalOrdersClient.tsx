    ...
    <div className="mx-auto mt-8 max-w-5xl space-y-3">
      {orders.map((order, index) => (
        <details key={order.id} open={index === 0} className="group overflow-hidden rounded-[1.5rem] border border-tan bg-white/75 shadow-soft">
          <summary className="flex cursor-pointer list-none flex-col gap-3 px-5 py-4 transition hover:bg-cream/60 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-3">
                <span className="font-serif text-2xl font-bold text-teal">#{order.orderNumber || order.id}</span>
                <span className="rounded-full bg-teal/10 px-3 py-1 text-xs font-extrabold uppercase tracking-[0.1em] text-teal">{order.status || "paid"}</span>
              </div>
              <p className="mt-1 text-sm text-ink/70">{formatDate(order.createdAt)} · {preview(order)}</p>
            </div>
            <div className="flex shrink-0 items-center gap-4">
              <span className="font-serif text-2xl font-bold text-teal">{money(order.total, order.currency)}</span>
          ...
          <div className="border-t border-tan px-5 pb-5 pt-4">
