...
const showTestDownloadPaths = process.env.NODE_ENV !== "production";
...
export default function ProductActions({ book }: ProductActionsProps) {
  ...
  return (
    <div>
      ...
      {showTestDownloadPaths ? (
        <div className="mt-6 rounded-2xl border border-tan bg-white/60 p-4 text-sm text-ink">
          <p className="font-extrabold text-teal">Development-only file test paths</p>
          ...
          <p className="mt-2 text-xs text-[#6b7d80]">For launch, paid ebooks and audiobook files stay private in Cloudflare R2 and deliver through signed links after checkout.</p>
        </div>
      ) : null}
